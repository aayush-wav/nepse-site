...
async def get_nepse_index():
    ...
    data = await asyncio.to_thread(nepse_client.get_nepse_index)
    if data:
...
async def get_sub_indices():
    ...
    if cached:
        return {"status": "ok", "data": cached}
    data = await asyncio.to_thread(nepse_client.get_nepse_sub_indices)
    if data:
...
async def get_all_indices():
    ...
    if cached:
        return {"status": "ok", "data": cached}
    data = await asyncio.to_thread(nepse_client.get_indices)
    if data:
...
async def get_sector_indices():
    ...
    if cached:
        return {"status": "ok", "data": cached}
    data = await asyncio.to_thread(nepse_client.get_sector_sub_indices)
    if data:
