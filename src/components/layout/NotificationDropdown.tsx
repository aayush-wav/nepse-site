...
import { Bell, TrendingUp, TrendingDown, Info, Clock, AlertTriangle } from 'lucide-react';
...
export default function NotificationDropdown({ onClose }: NotificationDropdownProps) {
  ...
  return (
    <div className="absolute top-full right-0 mt-2 w-80 bg-bg-surface border border-bg-border rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
      <div className="flex items-center justify-between p-4 border-b border-bg-border bg-bg-elevated/30">
        <div className="flex gap-4">
          <button 
            ...
            className={`font-syne font-bold text-sm transition-colors ${activeTab === 'SYSTEM' ? 'text-text-primary' : 'text-text-muted hover:text-text-primary'}`}
          >
            System
          </button>
          <button 
            onClick={() => setActiveTab('ALERTS')}
            className={`font-syne font-bold text-sm transition-colors ${activeTab === 'ALERTS' ? 'text-text-primary' : 'text-text-muted hover:text-text-primary'}`}
          >
            Price Alerts
          </button>
        </div>
      ...
      {activeTab === 'ALERTS' ? (
        <PriceAlertManager onClose={onClose} />
      ) : (
        <>
